...
const dump = async function({
  ...
  console.log(
    `Starting taking dump of [type]mysql [host]${host}:${port} [database]${database} to [temp-file]${temp_file_path}`
  );
  ...
  return new Promise((res, rej) => {
    exec(cmd, (err, stdout, stderr) => {
      ...
      console.log(
        `Done with taking dump of [type]mysql [host]${host}:${port} [database]${database} to [temp-file]${temp_file_path}`
      );
      res(temp_file_path);
    });
  });
